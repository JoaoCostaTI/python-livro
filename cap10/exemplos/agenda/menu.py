...
class Agenda(ListaÚnica):
    ...
    def adiciona_tipo(self, tipo):
        self.tiposTelefone.append(TipoTelefone(tipo))
    ...
    def ordena(self):
        super().sort(key=lambda dado: str(dado.nome))
...
class AppAgenda:
    ...
    def __init__(self):
        ...
        self.menu.adiciona_opção("Grava", self.grava)
        self.menu.adiciona_opção("Lê", self.lê)
        self.menu.adiciona_opção("Ordena", self.ordena)
    ...
    def novo_telefone(self, dados):
        ...
        dados.telefones.append(Telefone(telefone, tipo))
